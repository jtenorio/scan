...
class uniqueMultiColumnValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
			...
			if(''!=$criteria['condition'])
			{
				$criteria['condition'].= " AND ";
                        ...
                        if($column->type!=="integer"){
                           $data=$columnName;
                           $pos = strpos($column->dbType, 'numeric');
                           if($pos===false)
                            $criteria['condition'].=$this->caseSensitive ? "$columnName=:$attribute" : "LOWER($columnName)=LOWER(:$attribute)";
                           else
                             $criteria['condition'].=$this->caseSensitive ? "$columnName=:$attribute" : "$columnName=:$attribute";
                        }else
                            $criteria['condition'].=$this->caseSensitive ? "$columnName=:$attribute" : "$columnName=:$attribute";
		...
		if($exists)
		{
			...
			$message = $this->message!==null ? $this->message : MessageHandler::transformar('llave_compuesta','Default','Sistema',array('{message}'=>$message));
			foreach ($attributes as $attribute)
			{
