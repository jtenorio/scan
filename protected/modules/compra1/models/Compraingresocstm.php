...
class Compraingresocstm extends CActiveRecord
{
	...
	public function search()
	{
        ...
        public function buscaId($idCompra){

            if($idCompra==null)
                return '';

            $idCstm = $this->model()->findAll(
                    array(
                    'condition' => 'idcompra = :idcompra',
                    'params' => array(
                            ':idcompra' => $idCompra
                    )));
            foreach($idCstm as $idCstms){
                $idComprCstm = $idCstms['id'];
            }
            if($idComprCstm!=''){
                return trim($idComprCstm);
            }else
                return '';
        }

        public function buscaIdMaestroAsiento($idCompra){
            ...
            foreach($idCstm as $idCstms){
                $idMaestroAsiento = $idCstms['asientocompra'];
            }
            if($idMaestroAsiento!=''){
                return trim($idMaestroAsiento);
            }else
                return '';
        }
}