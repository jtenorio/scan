...
<table border="1">
    ...
    ?>
     <tr>
        ...
        <td>
            <input type="text" name="abono_<?php echo $idCompra?>_<?php echo $compra['secuencial']?>" id="abono_<?php echo $idCompra?>_<?php echo $compra['secuencial']?>" value="<?php echo $compra['apagar']?>" size="6" onblur="
               ...
               if(this.value <= <?php echo $compra['saldo']?>) {
                ...
                sendPage('pagos', '<?php echo Yii::app()->request->baseUrl; ?>/index.php/contabilidad/maestrochequeproveedor/detalle/cuenta/'+$('#Maestroasiento_idcuentabancaria').val()+'/idProveedor/'+$('#idProveedor').val(), 'crearDetalleAsiento');
                }else{
        ...
        <td><input type="checkbox" name="cancela_<?php echo $idCompra?>" id="cancela_<?php echo $idCompra?>" value="1" 
                   onclick="
                       if($('input[name=cancela_<?php echo $idCompra?>]').is(':checked')){
                           $('#abono_<?php echo $idCompra?>_<?php echo $compra['secuencial']?>').val(<?php echo $compra['saldo']?>);
