 ...
 * @property integer $anioejercicio
...
class Parametrocontabilidad extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('anioejercicio, reporteingreso, reporteegreso, reportecancelacion, reporteform104, reporteform103, direccionrespaldo', 'length', 'max'=>254),
			//array('', 'length', 'max'=>4),
	...
	public function search()
	{
        ...
        public static function getNuevoNumeroAsiento($idEmpresa)
        {
            
            $actual = Parametrocontabilidad::model()->findByAttributes(array('idempresa'=>$idEmpresa));
            return $actual->numeroasiento;
//            $maxNumber = Yii::app()->db->createCommand()
//              ->select('max(numeroasiento) as max')
//              ->from('parametrocontabilidad')
...
//            return $maxNumber + 1;
        ...
        public static function updateNumeroAsiento($idEmpresa)
        {
            $actual = Parametrocontabilidad::model()->findByAttributes(array('idempresa'=>$idEmpresa));
            $actual->numeroasiento = $actual->numeroasiento + 1;
            $actual->save();
        }
        
        public static function getParamtrosByEmpresa($idEmpresa)
        {
            $actual = Parametrocontabilidad::model()->findByAttributes(array('idempresa'=>$idEmpresa));
            return $actual;
        }
